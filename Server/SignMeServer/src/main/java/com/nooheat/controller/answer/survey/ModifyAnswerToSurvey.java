 ...
 */
@API(category = Category.SURVEY, summary = "설문조사 응답 수정", requestBody = "answers : []", successCode = 200, failureCode = 400, etc = "잘못된 요청 : 400, 비로그인 : 401")
@URIMapping(uri = "/answer/survey/:letterNumber", method = HttpMethod.PUT)
public class ModifyAnswerToSurvey implements Handler<RoutingContext> {
    @Override
    public void handle(RoutingContext ctx) {
        ...
        String answerDate = DateTime.getDateNow();

        if (RequestManager.paramValidationCheck(answers) == false) {
            res.setStatusCode(400).end();
            return;
        }
        ...
        try {
            success = survey.modifyAnswer(uid, answers, answerDate);
        } catch (SQLException e) {
            e.printStackTrace();
            res.setStatusCode(500).end();
            return;
